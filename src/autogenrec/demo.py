"""
Full System Demo for AutoGenRec.
...
def run_demo() -> None:
    ...
    researcher_entity_id = researcher_mask.entity_id or "researcher_001"
    ...
    project = academia.create_project(
        ...
        lead_researcher_id=researcher_entity_id,
    )
    ...
    paper = academia.create_publication(
        ...
        author_ids=[researcher_entity_id],
    ...
    _archive = academia.archive_publication(paper.id)
    _project_archive = academia.archive_project(project.id)
    ...
    assert sdk_code is not None and sdk_code.code is not None
    ...
    # Execute transfer
    _transfer_result = exchange.transfer(
        platform_account.id,
    ...
    assert researcher_final is not None
    print("\n[Final Balance]")
